...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        allpossibles=util.Counter()

        for oldPos,belief in self.getBeliefDistribution().items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos,pr in newPosDist.items():
                allpossibles[newPos]+=pr*belief
        allpossibles.normalize()
        self.beliefs=allpossibles
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        w=False
        for b in allPossible.values():
            if b!=0:
                w=True
        for i in range(self.numParticles):
            if w:#(len(allPossible)==1 and 0 in allPossible.values()):
                #print("allPossible",allPossible)
                #print("legalPositions",self.legalPositions)
                newParticles.append(util.sample(allPossible))
            else:
                #print("legalPositions",legalPositions)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #AB=itertools.product(A,A,A)
        #ABP=itertools.permutations(A)
        #while True:
         #   x=AB.next()
          #  if x==None:
           #     break
        ...
        allLegalPositions = itertools.product(self.legalPositions,repeat=self.numGhosts)

        #print ("num:",self.numGhosts)
        array=[]

        for x in allLegalPositions:
            array.append(x)
        random.shuffle(array)
        self.particles=[]
        #while True:
            #x=allLegalPositions.next()
            #print("x:",x)
            #if x==None:
                #break
            #array.append(x)
        ...
        while True:
            ...
            #print ("j:",j)
            #print("i",i)
            self.particles.append(array[j])#Domain
            i=i+1
            j=j+1

        #print("len:",len(self.particles))
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] == None:
                    ...
                    #print ("jail",self.getJailPosition(i))
                else:

                    w*=emissionModels[i][util.manhattanDistance(pacmanPosition,g)]
            allPossible[tuple(newPos)]+=w
        newParticles=[]
        items=allPossible.items()
        dist = [i[1] for i in items]

        #check for weight==0 cases
        if (allPossible.totalCount()==0 and sum(dist)==0):
            self.initializeParticles()
            return
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        allPossible=util.Counter()
        for p in self.particles:allPossible[p]+=1
        allPossible.normalize()
        return allPossible
