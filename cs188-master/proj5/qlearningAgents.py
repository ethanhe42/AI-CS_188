...
class QLearningAgent(ReinforcementAgent):
    ...
    def setQValue(self, state, action, value):
        """
        :param state: State action is taken in
        :param action: Action taken
        :param value: QValue to be set
        :return: None
        """
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        return self.getQValue(state=state, action=self.computeActionFromQValues(state))
    ...
    def update(self, state, action, nextState, reward):
        ...
        update_value = current + self.discount * (reward + self.discount * next_state_max - current)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        current = self.getQValue(state = state, action = action)
        next_state_max = self.getValue(nextState) if nextState != "TERMINAL_STATE" else 0
        diff = reward + self.discount * next_state_max - current
        w = self.getWeights()
        feat = self.featExtractor.getFeatures(state, action)
        multiplier = self.alpha * diff
